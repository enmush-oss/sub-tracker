/**
 * 엔드투엔드: 실제 샘플 명세서 파일 → 파싱 → 정기결제 탐지.
 * samples/ 의 EUC-KR 파일이 제대로 디코딩되는지가 이 앱의 생명줄이라 실물로 확인한다.
 */
...
function fileFrom(relPath: string): File {
  const buf = readFileSync(resolve(ROOT, relPath))
  return new File([new Uint8Array(buf)], relPath.split('/').pop()!, { type: 'text/csv' })
}

/** 샘플은 생성 시점 기준 상대 날짜다. 고정 날짜를 박으면 시간이 지나 테스트가 깨지므로
 *  명세서의 마지막 거래 다음날을 "오늘"로 삼는다. */
...
async function pipeline(relPath: string) {
  const parsed = await readCsvFile(fileFrom(relPath))
  const mapping = { ...suggestMapping(parsed.headers), ...parsed.suggested } as ColumnMapping
  const txns = rowsToTxns(parsed.rows, mapping, { defaultCurrency: 'KRW', sourceFile: relPath })
  const today = todayFrom(txns.map((t) => t.date))
  const series = detectSeries(txns, { today })
  return { parsed, mapping, txns, series, today }
}

describe('샘플 명세서 엔드투엔드', () => {
  for (const [label, path] of [
    ['UTF-8', 'samples/sample-card-statement.csv'],
    ['EUC-KR', 'samples/sample-card-statement-euckr.csv'],
  ] as const) {
    describe(label, () => {
      it('한글 가맹점명이 깨지지 않고 디코딩된다', async () => {
        const { parsed } = await pipeline(path)
      ...
      it('한국어 헤더로 컬럼 매핑을 자동 추측한다', async () => {
        const { mapping } = await pipeline(path)
      ...
      it('따옴표 안의 쉼표가 든 금액을 온전히 파싱한다', async () => {
        const { txns } = await pipeline(path)
      ...
      it('정기 구독 10건을 높은 확신도로 찾아낸다', async () => {
        const { series } = await pipeline(path)
